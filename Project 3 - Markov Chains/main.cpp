...
bool assertTrue(const string& nameOfTest, bool actual) {
    if (actual == true) {
        // Green colored text
        cout << "PASSED " << nameOfTest << ": test successfully resulted in 'true'" << endl;
        testCount++;
        return true;
    }
    // Red colored text
    cout << "FAILED " << nameOfTest << ": test unsuccessfully resulted in 'false'" << endl;
    return false;
}
...
bool assertValidSentence(const string & nameOfTest, const string & actual) {
    ...
    if (isupper(actual[0]) && (lastChar == '.' || lastChar == '?' || lastChar == '!')) {
        for (int i = 1; i < actual.size() - 1; ++i) {
            if (actual[i] == ' ') {
                // Green colored text
                cout << "PASSED " << nameOfTest << ": valid sentence '" << actual << "'" << endl;
                testCount++;
                return true;
            }
        }
    ...
    // Red colored text
    cout << "FAILED " << nameOfTest << ": invalid sentence '" << actual << "'" << endl;
    return false;
}
    ...
    if (expected.size() == actual.size()) {
        for (size_t i = 0; i < expected.size(); i++) {
            if (expected[i] != actual[i]) {
                cout << "FAILED "
                    << nameOfTest << ": expected '" << expected[i] << "' but actually '" << actual[i] << "'" << endl;
                return false;
            }
        }
...
float markov_chain_equivalence(map<string, vector<string>> ans, map<string, vector<string>> supplied) {
    ...
    for (auto it = ans.begin(); it != ans.end(); ++it) if (supplied[it->first] != ans[it->first]) running_total--;

    float percentage = (float)running_total / (float)size;
    if (percentage > .6) {
        cout << "PASSED " << "Markov Chain is at least 60% equivalent.";
        testCount++;
    }
    else cout << "FAILED " << "Markov Chain is NOT at least 60% equivalent.";
    return percentage;
}
