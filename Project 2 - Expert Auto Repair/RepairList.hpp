...
{
    if (_nowServicing != _dailyList.end()) ++_nowServicing;	
}
...
{
    return std::next(_dailyList.begin(), number - 1)->vehicle();
}
...
{
    double total = 0.0;
    for (auto& curr : _dailyList) {
        if (curr.cost() > total) total = curr.cost();
    }
    return total;
}
